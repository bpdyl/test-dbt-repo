...
{{ config(
    materialized='view',
    alias='V_STG_F_INV_RTV_SUP_ILD_B',
    schema='DW_STG_V',
    tags=['f_inv_rtv_sup_ild_ld']
) }}
-- depends_on: {{ ref('V_RECON_F_INV_RTV_SUP_ILD_B') }}
SELECT
        RTV.POST_DT                                                       AS POST_DT
        ,RTV.TXN_DT                                                       AS TXN_DT
        ,RTV.LOC_ID                                                       AS LOC_ID
        ,RTV.SUP_ID                                                       AS SUP_ID
        ,RTV.ITM_ID                                                       AS ITM_ID
        ,RTV.RTV_RSN                                                      AS RTV_RSN
    ...
    FROM {{ source(src_name,'LND_F_INV_RTV_SUP_ILD_B') }} RTV
