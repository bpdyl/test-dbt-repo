...
{{ config(
    materialized='view',
    alias='V_STG_D_PRD_SIZE_LU',
    schema='DW_STG_V',
    tags=['d_prd_size_ld']
) }}
SELECT
     TRIM(SIZ.SIZE_ID)                                             AS SIZE_ID
    ,TRIM(SIZ.SIZE_DESC)                                           AS SIZE_DESC
    ,TRIM(SIZ.SIZE_ID)                                             AS SIZE_NUM
    ,COALESCE(TRIM(SORT_LU.SIZE_SORT_NUM),TRIM(SIZ.SIZE_ID))       AS SIZE_SORT_NUM
FROM {{ source(src_name, 'LND_D_PRD_SIZE_LU') }} SIZ
LEFT JOIN {{ source(src_name, 'LND_D_PRD_SIZE_SORT_NUM_LU') }} SORT_LU ON TRIM(SIZ.SIZE_ID) = TRIM(SORT_LU.SIZE_ID)
