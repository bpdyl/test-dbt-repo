...
{{ config(
    materialized='view',
    alias='V_STG_D_PRD_GRP_LU',
    schema='DW_STG_V',
    tags=['d_prd_grp_ld']
) }}
SELECT
     TRIM(GRP.GRP_ID)       AS GRP_ID
    ,TRIM(GRP.GRP_ID)       AS GRP_NUM
    ,TRIM(GRP.GRP_DESC)     AS GRP_DESC
    ,TRIM(GRP.DIV_ID)       AS DIV_ID
FROM {{ source(src_name, 'LND_D_PRD_GRP_LU') }} GRP
