...
{{ config(
    materialized='view',
    alias='V_STG_D_ORG_CHNL_LU',
    schema='DW_STG_V',
    tags=['d_org_chnl_ld']
) }}

SELECT
     TRIM(CHNL.CHNL_ID)                                          AS CHNL_ID
    ,TRIM(CHNL.CHNL_ID)                                          AS CHNL_NUM
    ,TRIM(CHNL.CHNL_DESC)                                        AS CHNL_DESC
FROM {{ source(src_name,'LND_D_ORG_CHNL_LU') }} CHNL 