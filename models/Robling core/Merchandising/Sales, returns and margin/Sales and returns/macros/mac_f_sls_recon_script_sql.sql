{% macro mac_f_sls_recon_script_sql() %}
    {# 
        OVERVIEW:
    ...
    recon_steps:
      - recon_step: 0
        ...
        sql: |
            ...
            /*This select loads Sales data from landing tables for current business date */
            SELECT 'Sales'                      AS FACT_TYP
                ,'LND'                          AS SRC_SYSTEM
                ,'V_RECON_F_SLS_TXN_LN_ITM_B'   AS SRC_TABLE
                ,'${CURR_DAY}'                  AS LOAD_DT
                ,TO_DATE(SRC.TXN_TS)            AS TXN_DT
                ,SRC.LOC_ID                     AS LOC_ID
                ,SRC.LCL_CNCY_CDE               AS LCL_CNCY_CDE
                ,SUM(SRC.F_SLS_QTY)             AS FACT_QTY
                ,SUM(F_SLS_RTL_LCL)             AS F_SLS_RTL_LCL
                ,NULL                           AS FACT_CST_LCL
                ,NULL                           AS ATTR_1_NAME
                ,NULL                           AS ATTR_1_VALUE
                ,CURRENT_TIMESTAMP()            AS RCD_INS_TS
                ,CURRENT_TIMESTAMP()            AS RCD_UPD_TS
            FROM {{ source('SLS_RECON_SRC_STG','V_RECON_F_SLS_TXN_LN_ITM_B') }} SRC
            GROUP BY LOAD_DT
                    ,TXN_DT
                    ,LOC_ID
                    ,LCL_CNCY_CDE
      ...
      - recon_step: 0
        ...
        sql: |
            ...
            SELECT 'Sales'                    AS FACT_TYP
                ,'STG_V'                      AS SRC_SYSTEM
                ,'V_STG_F_SLS_TXN_LN_ITM_B'   AS SRC_TABLE
                ,'${CURR_DAY}'                AS LOAD_DT
                ,TO_DATE(SRC.TXN_TS)          AS TXN_DT
                ,SRC.LOC_ID                   AS LOC_ID
                ,SRC.LCL_CNCY_CDE             AS LCL_CNCY_CDE
                ...
                ,CURRENT_TIMESTAMP()          AS RCD_INS_TS
                ,CURRENT_TIMESTAMP()          AS RCD_UPD_TS
            FROM {{ source('SLS_RECON_SRC_STG','V_STG_F_SLS_TXN_LN_ITM_B') }} SRC
            GROUP BY LOAD_DT
                    ,TXN_DT
                    ,LOC_ID
                    ,LCL_CNCY_CDE
      ...
      - recon_step: 1
        ...
        sql: |
            ...
            SELECT 'Sales'                    AS FACT_TYP
                ,'DWH'                        AS SRC_SYSTEM
                ,'DWH_F_SLS_TXN_LN_ITM_B'     AS SRC_TABLE
                ,'${CURR_DAY}'                AS LOAD_DT
                ,SRC.TXN_DT                   AS TXN_DT
                ,SRC.LOC_ID                   AS LOC_ID
                ,SRC.LCL_CNCY_CDE             AS LCL_CNCY_CDE
                ,SUM(SRC.F_SLS_QTY)           AS F_SLS_QTY
                ,SUM(SRC.F_SLS_RTL_LCL)       AS F_SLS_RTL_LCL
                ,NULL                         AS FACT_CST_LCL
                ,NULL                         AS ATTR_1_NAME
                ,NULL                         AS ATTR_1_VALUE
                ,CURRENT_TIMESTAMP()          AS RCD_INS_TS
                ,CURRENT_TIMESTAMP()          AS RCD_UPD_TS
            FROM {{ source('SLS_RECON_SRC_DWH','V_DWH_F_SLS_TXN_LN_ITM_B') }} SRC
      ...
      - recon_step: 2
        ...
        sql: |
            ...
            SELECT 'Sales'                    AS FACT_TYP
                ...
                ,CURRENT_TIMESTAMP            AS RCD_INS_TS
                ,CURRENT_TIMESTAMP            AS RCD_UPD_TS
            FROM {{ source('SLS_RECON_SRC_DM','DM_F_MEAS_FACT_ILD_B') }} SRC
            WHERE SRC.FACT_CDE = 'SLS' 
                AND SRC.POST_DT = '${CURR_DAY}'
                GROUP BY LOAD_DT
                        ,TXN_DT
                        ,LOC_ID
                        ,LCL_CNCY_CDE    
