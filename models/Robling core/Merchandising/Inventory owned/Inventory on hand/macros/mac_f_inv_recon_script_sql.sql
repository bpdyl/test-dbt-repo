{% macro mac_f_inv_recon_script_sql() %}
    {# 
        OVERVIEW:
    ...
    recon_steps:
      - recon_step: 0
        ...
        sql: |
            ...
            SELECT 'Inventory On-Hand'          AS FACT_TYP
                ,'LND'                          AS SRC_SYSTEM
                ,'V_RECON_F_INV_ILD_B'          AS SRC_TABLE
                ,'${CURR_DAY}'                  AS LOAD_DT
                ,'${CURR_DAY}'                  AS TXN_DT
                ,SRC.LOC_ID                     AS LOC_ID
                ,SRC.LCL_CNCY_CDE               AS LCL_CNCY_CDE
                ,SUM(SRC.F_OH_QTY)              AS FACT_QTY
                ,SUM(SRC.F_OH_RTL_LCL)          AS FACT_RTL_LCL
                ,SUM(SRC.F_OH_CST_LCL)          AS FACT_CST_LCL
                ,NULL                           AS ATTR_1_NAME
                ,NULL                           AS ATTR_1_VALUE
                ,CURRENT_TIMESTAMP()            AS RCD_INS_TS
                ,CURRENT_TIMESTAMP()            AS RCD_UPD_TS
            FROM {{ source('INV_RECON_SRC_STG','V_RECON_F_INV_ILD_B') }} SRC
            GROUP BY LOAD_DT
                    ,TXN_DT
                    ,LOC_ID
                    ,LCL_CNCY_CDE
      ...
      - recon_step: 0
        ...
        sql: |
            ...
            SELECT 'Inventory On-Hand'        AS FACT_TYP
                ,'STG_V'                      AS SRC_SYSTEM
                ,'V_STG_F_INV_ILD_B'          AS SRC_TABLE
                ,'${CURR_DAY}'                AS LOAD_DT
                ,'${CURR_DAY}'                AS TXN_DT
                ,SRC.LOC_ID                   AS LOC_ID
                ,SRC.LCL_CNCY_CDE             AS LCL_CNCY_CDE
                ...
                ,CURRENT_TIMESTAMP()          AS RCD_INS_TS
                ,CURRENT_TIMESTAMP()          AS RCD_UPD_TS
            FROM {{ source('INV_RECON_SRC_STG','V_STG_F_INV_ILD_B') }} SRC
            GROUP BY LOAD_DT
                    ,TXN_DT
                    ,LOC_ID
                    ,LCL_CNCY_CDE
      ...
      - recon_step: 1
        ...
        sql: |
            ...
            SELECT 'Inventory On-Hand'        AS FACT_TYP
                ...
                ,CURRENT_TIMESTAMP()          AS RCD_INS_TS
                ,CURRENT_TIMESTAMP()          AS RCD_UPD_TS
            FROM {{ source('INV_RECON_SRC_DWH','DWH_F_INV_ILD_B') }} SRC
            WHERE SRC.EFF_END_DT >= (SELECT DISTINCT TO_DATE(EFF_START_DT) FROM
                            {{ source('INV_RECON_SRC_STG','V_RECON_F_INV_ILD_B') }})
            GROUP BY LOAD_DT
                    ,TXN_DT
                    ,LOC_ID
                    ,LCL_CNCY_CDE
      ...
      - recon_step: 2
        ...
        sql: |
            ...
            SELECT 'Inventory On-Hand'        AS FACT_TYP
                ...
                ,CURRENT_TIMESTAMP()          AS RCD_INS_TS
                ,CURRENT_TIMESTAMP()          AS RCD_UPD_TS
            FROM {{ source('INV_RECON_SRC_DM','DM_F_MEAS_FACT_ILD_B') }} SRC
            WHERE SRC.FACT_CDE = 'OH' 
                AND SRC.CLOSED_DT >= (SELECT DISTINCT TO_DATE(EFF_START_DT) FROM
                            {{ source('INV_RECON_SRC_STG','V_STG_F_INV_ILD_B') }})
            GROUP BY LOAD_DT
                    ,TXN_DT
                    ,LOC_ID
                    ,LCL_CNCY_CDE    
