{{ config(
    ...
    transient=false,
    alias='RIG_F_INV_IMMNT_STKOUT_ILD_B',
    schema='DW_RIG',
    ...
    post_hook=["{{ log_dml_audit(this, ref('TMP_RIG_F_INV_IMMNT_STKOUT_ILD_B'), 'INSERT') }}","{{ log_script_success(this) }}"]
) }}
...
FROM 
    {{ ref('TMP_RIG_F_INV_IMMNT_STKOUT_ILD_B') }} SRC