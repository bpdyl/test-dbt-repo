...
{{
    config(
        ...
        tags = ['dm_f_rig_inv_stkout_meas_fact_ild'],
        fact_cde='RIG_INV_STKOUT',
        pre_hook=["{{ start_script('dm_f_rig_inv_stkout_meas_fact_ild','RUNNING','NONE') }}"],
        post_hook = ["{{ log_script_success(this) }}"]
    )
}}
SELECT
    'RIG_INV_STKOUT'                                           AS FACT_CDE
    ...
    ,SRC.F_STKOUT_COUNT                                        AS F_FACT_QTY2 
    ,SRC.F_STKOUT_DAY_COUNT                                    AS F_FACT_QTY3 
    ,SRC.F_STKOUT_WEEKEND_DAY_COUNT                            AS F_FACT_QTY4 
    ,CURRENT_TIMESTAMP                                         AS RCD_INS_TS                                
    ,CURRENT_TIMESTAMP                                         AS RCD_UPD_TS
FROM {{ ref('TMP_RIG_F_INV_STKOUT_ILD_B') }} SRC
